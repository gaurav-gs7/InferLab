...
func run(args []string, stdout, stderr io.Writer) int {
	...
	trackedStderr := &trackingWriter{writer: stderr}
	code := runCommand(args, trackedStdout, trackedStderr)
...
func writeText(writer io.Writer, value string) {
	_, _ = fmt.Fprint(writer, value)
}

func writef(writer io.Writer, format string, arguments ...any) {
	_, _ = fmt.Fprintf(writer, format, arguments...)
}

func writeln(writer io.Writer, arguments ...any) {
...
func runCommand(args []string, stdout, stderr io.Writer) int {
	...
	case "help", "-h", "--help":
		...
		return 0
	case "version":
		writef(stdout, "inferlab %s (commit=%s, built=%s)\n", buildinfo.Version, buildinfo.Commit, buildinfo.Date)
		return 0
	case "change":
...
func runEvaluate(args []string, stdout, stderr io.Writer) int {
	if len(args) < 1 || len(args) > 2 {
		writeText(stderr, evaluateUsage)
		return 2
	}
	output := ""
	if len(args) == 2 {
...
func runGate(args []string, stdout, stderr io.Writer) int {
	...
	if len(args) == 3 && args[0] == "evaluation" && args[1] == "validate" {
		...
		if err != nil {
			writef(stderr, "open gate evaluation: %v\n", err)
			return 1
		}
		defer func() { _ = file.Close() }()
		evaluation, err := gate.DecodeEvaluation(file)
		if err != nil {
			writef(stderr, "invalid gate evaluation: %v\n", err)
			return 1
		}
	...
	if len(args) == 3 && args[0] == "result" && (args[1] == "validate" || args[1] == "digest") {
		file, err := os.Open(args[2])
		if err != nil {
			writef(stderr, "open gate result: %v\n", err)
			return 1
		}
		defer func() { _ = file.Close() }()
		result, err := gate.DecodeResult(file)
		if err != nil {
			writef(stderr, "invalid gate result: %v\n", err)
			return 1
		}
		digest, err := gate.ResultDigest(result)
		if err != nil {
			writef(stderr, "digest gate result: %v\n", err)
			return 1
		}
		if args[1] == "digest" {
			writeln(stdout, digest)
		} else {
			writef(stdout, "valid %s %s %s\n", result.Evaluation, digest, result.Decision)
		}
		return 0
	}
	writeText(stderr, gateUsage)
	return 2
}
...
func evaluateGate(path, output string, stdout, stderr io.Writer) int {
	...
	if err != nil {
		writef(stderr, "evaluate gate: %v\n", err)
		return 1
	}
	encoded, err := gate.CanonicalResultJSON(result)
	if err != nil {
		writef(stderr, "encode gate result: %v\n", err)
		return 1
	}
	if output == "" {
		writeln(stdout, string(encoded))
	} else if err := atomicWrite(output, append(encoded, '\n'), 0o644); err != nil {
		writef(stderr, "write gate result: %v\n", err)
		return 1
	} else {
...
func runSafetyCase(args []string, stdout, stderr io.Writer) int {
	if len(args) == 3 && args[0] == "assemble" {
		...
		if err != nil {
			writef(stderr, "assemble safety case: %v\n", err)
			return 1
		}
		...
		if err != nil {
			writef(stderr, "encode safety case: %v\n", err)
			return 1
		}
		if err := atomicWrite(args[2], append(encoded, '\n'), 0o644); err != nil {
			writef(stderr, "write safety case: %v\n", err)
			return 1
		}
	...
	if len(args) == 2 && (args[0] == "validate" || args[0] == "digest") {
		...
		if err != nil {
			writef(stderr, "invalid safety case: %v\n", err)
			return 1
		}
		digest, _ := safetycase.ManifestDigest(manifest)
		if args[0] == "digest" {
	...
	if len(args) == 4 && args[0] == "sign" {
		...
		if err := atomicWrite(args[3], append(encoded, '\n'), 0o644); err != nil {
			writef(stderr, "write safety-case signature: %v\n", err)
			return 1
		}
	...
	if (len(args) == 4 || len(args) == 5) && args[0] == "verify" {
		manifest, err := decodeSafetyCaseManifest(args[1])
		if err != nil {
			writef(stderr, "verify safety case: %v\n", err)
			return 1
		}
		...
		writef(stdout, "verified %s %s %s\n", manifest.Name, signature.ManifestDigest, manifest.Decision)
		return 0
	}
	writeText(stderr, safetyCaseUsage)
	return 2
}
...
func runAdapter(args []string, stdout, stderr io.Writer) int {
	...
	if len(args) == 3 && args[0] == "normalize" {
		...
		file, err := os.Open(args[2])
		if err != nil {
			writef(stderr, "open adapter input: %v\n", err)
			return 1
		}
		defer func() { _ = file.Close() }()
		input, err := adapter.DecodeInput(file)
		if err != nil {
			writef(stderr, "invalid adapter input: %v\n", err)
			return 1
		}
		report, err := implementation.Normalize(input)
		if err != nil {
			writef(stderr, "normalize evidence: %v\n", err)
			return 1
		}
		encoded, err := adapter.CanonicalJSON(report)
		if err != nil {
			writef(stderr, "encode normalized report: %v\n", err)
			return 1
		}
		writeln(stdout, string(encoded))
		return 0
	...
	if len(args) == 2 && (args[0] == "validate" || args[0] == "digest") {
		...
		if err != nil {
			writef(stderr, "open normalized report: %v\n", err)
			return 1
		}
		defer func() { _ = file.Close() }()
		report, err := adapter.DecodeNormalizedReport(file)
		if err != nil {
			writef(stderr, "invalid normalized report: %v\n", err)
			return 1
		}
		digest, err := adapter.Digest(report)
		if err != nil {
			writef(stderr, "digest normalized report: %v\n", err)
			return 1
		}
		if args[0] == "digest" {
			writeln(stdout, digest)
	...
	writeText(stderr, adapterUsage)
	return 2
}

func runEvidence(args []string, stdout, stderr io.Writer) int {
	if len(args) != 2 || (args[0] != "validate" && args[0] != "digest") {
		writeText(stderr, evidenceUsage)
		return 2
	}
	file, err := os.Open(args[1])
	if err != nil {
		writef(stderr, "open evidence envelope: %v\n", err)
		return 1
	}
	defer func() { _ = file.Close() }()
	envelope, err := evidence.Decode(file)
	if err != nil {
		writef(stderr, "invalid evidence envelope: %v\n", err)
		return 1
	}
	digest, err := evidence.Digest(envelope)
	if err != nil {
		writef(stderr, "digest evidence envelope: %v\n", err)
		return 1
	}
	if args[0] == "digest" {
		writeln(stdout, digest)
...
func runRuntime(args []string, stdout, stderr io.Writer) int {
	...
	if err != nil {
		writef(stderr, "inspect runtime signature: %v\n", err)
		return 1
	}
	writef(stdout, "valid %s %s unknown=%d\n", digest, signature.Origin, len(unknown))
	return 0
}

func runChange(args []string, stdout, stderr io.Writer) int {
	if len(args) != 2 || (args[0] != "validate" && args[0] != "digest") {
		writeText(stderr, changeUsage)
		return 2
	}

	file, err := os.Open(args[1])
	if err != nil {
		writef(stderr, "open inference change: %v\n", err)
		return 1
	}
	defer func() { _ = file.Close() }()
	...
	if err != nil {
		writef(stderr, "invalid inference change: %v\n", err)
		return 1
	}
	digest, err := change.Digest(document)
	if err != nil {
		writef(stderr, "digest inference change: %v\n", err)
		return 1
	}
